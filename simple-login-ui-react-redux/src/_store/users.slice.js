...
function createExtraActions() {
  ...
  function getAudit() {
    return createAsyncThunk(
      `${name}/audit`,
      async ({user,page,limit}) => await fetchWrapper.get(`${baseUrl}/audit/${user._id}?page=${page}&limit=${limit}`)
    );
  }
...
function createExtraReducers() {
  return (builder) => {
    ...
    function getAudit() {
      ...
      builder
        .addCase(pending, (state) => {
          state.totalUsers = { loading: true };
        })
        .addCase(fulfilled, (state, action) => {
          state.totalUsers = { value: action.payload };
        })
        .addCase(rejected, (state, action) => {
          state.totalUsers = { error: action.error };
        });
    }
